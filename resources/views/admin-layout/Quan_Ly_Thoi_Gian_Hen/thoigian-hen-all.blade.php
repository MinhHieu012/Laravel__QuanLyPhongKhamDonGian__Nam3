    ...
    <div>
        ...
        <div class="table1">
            <table id="hosobacsi" class="table table-bordered border-dark" style="width: 100%">
                ...
                <!-- thân bảng -->
                <tbody>
                @forelse($time as $time)
                    <tr>
                        <td>{{ $time->id }}</td>
                        <td>{{ $time->types }}</td>
                        <td>{{ $time->times }}</td>
                        <td>
                            <form action="{{ url('admin/quanlythoigianhen/edit/' . $time->id) }}"
                                  method="GET">
                                <button type="submit" class="btn btn-outline-warning">Sửa</button>
                            </form>
                        </td>
                        <td>
                            <form action="{{ url('admin/quanlythoigianhen/delete/' . $time->id) }}" method="GET">
                                <button type="submit" onclick="return confirm('Bạn có chắc chắn muốn xóa thời gian hẹn này?')"
    ...
    $(document).ready(function () {
        ...
        $('#hosobacsi').DataTable({
            language: {
                ...
                emptyTable: "Chưa có mốc thời gian hẹn nào",
                paginate: {
                    first: "Trang đầu",
