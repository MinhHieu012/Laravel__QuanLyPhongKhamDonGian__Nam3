...
@section('content2')
    <div>
        ...
        <button type="button" style="position: relative; right: -270px; top: 40px" class="btn btn-warning"
                onclick="window.location.href='{{URL::asset('/admin/quanlykhachhang/lock')}}';">Xem các tài khoản khách
            hàng bị khóa
        </button>
        @if (session('success'))
            <script>
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="{{ url('/admin/quanlykhachhang/delete/'. $accounts->id)}}" method="GET">
                                <button type="submit"
                                        onclick="return confirm('Bạn có chắc chắn muốn xóa tài khoản (Thao tác này không thể quay lại!)?')"
                                        class="btn btn-outline-danger">Xóa tài khoản
                                </button>
                            </form>
                        </td>
                        ...
                        <td>
                            <form action="{{ url('/admin/quanlykhachhang/lock/' . $accounts->id) }}" method="POST">
                                <button type="submit" class="btn btn-outline-danger"
                                        onclick="confirm('Bạn muốn khóa tài khoản này?')">Khóa tài khoản
                                </button>
                            </form>
                        </td>
                    </tr>
